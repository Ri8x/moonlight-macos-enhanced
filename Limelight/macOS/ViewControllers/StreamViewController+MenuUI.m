...
- (void)rebuildStreamMenu {
    ...
    NSMenuItem *toggleBallItem = [[NSMenuItem alloc] initWithTitle:MLString(@"Show Floating Ball in Fullscreen", nil) action:@selector(toggleFullscreenControlBallFromMenu:) keyEquivalent:@""];
    ...
    if (!currentFpsIsStandard) {
         NSString *title = [NSString stringWithFormat:MLString(@"Current: %@", nil), [NSString stringWithFormat:@"%d FPS", currentFps]];
    ...
    NSMenuItem *autoBitrateItem = [[NSMenuItem alloc] initWithTitle:MLString(@"Auto", nil) action:@selector(selectBitrateFromMenu:) keyEquivalent:@""];
    ...
    NSMenuItem *autoItem = [[NSMenuItem alloc] initWithTitle:MLString(@"Auto", nil) action:@selector(selectConnectionMethodFromMenu:) keyEquivalent:@""];
    ...
    NSMenuItem *copyLogsItem = [[NSMenuItem alloc] initWithTitle:MLString(@"Copy Logs", nil) action:@selector(copyLogsFromMenu:) keyEquivalent:@""];
...
- (void)selectCustomResolutionFromMenu:(id)sender {
    ...
    alert.informativeText = MLString(@"Custom Resolution and Frame Rate hint", nil);
...
- (void)selectCustomFpsFromMenu:(id)sender {
    NSAlert *alert = [[NSAlert alloc] init];
    alert.messageText = MLString(@"Custom FPS", nil);
    alert.informativeText = MLString(@"Custom FPS hint", nil);
    [alert addButtonWithTitle:MLString(@"OK", nil)];
    [alert addButtonWithTitle:MLString(@"Cancel", nil)];
